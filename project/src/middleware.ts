...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith(PENDING_APPROVAL_PATH)) {
    ...
    if (isPartnerEnabled) return NextResponse.redirect(new URL("/producer", request.url));
  ...
  if (pathname.startsWith(ONBOARDING_PATH)) {
    if (!user) return redirectToLogin(request, pathname);
    if (user.profileCompleted) {
      const target = isSuperadmin ? "/admin" : isPartnerEnabled ? "/producer" : PENDING_APPROVAL_PATH;
  ...
  if (pathname.startsWith("/producer")) {
    ...
    if (isSuperadmin) return NextResponse.redirect(new URL("/admin", request.url));
    if (!isPartnerEnabled) return NextResponse.redirect(new URL(PENDING_APPROVAL_PATH, request.url));
    return NextResponse.next();
  }
  ...
  if (pathname.startsWith("/admin")) {
    ...
    if (!canAccessAdmin) {
      return NextResponse.redirect(
        new URL(isPartnerEnabled ? "/producer" : PENDING_APPROVAL_PATH, request.url)
      );
    }
    return NextResponse.next();
  }
