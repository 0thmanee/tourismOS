...
function buildPath(type: UploadMediaType, userId: string, suggestedName: string): string {
  const segment = typeToPathSegment(type);
  const rawExt = suggestedName.split(".").pop()?.toLowerCase() || "jpg";
  const config = CONFIG[type];
  const allowedExts =
    type === "certificationDocuments" ? DOC_EXTS
    : type === "trainingProgramMedia" ? TRAINING_EXTS
    : IMAGE_EXTS;
  const safeExt = allowedExts.includes(rawExt) ? rawExt : (type === "trainingProgramMedia" ? "mp4" : type === "certificationDocuments" ? "pdf" : "jpg");
...
export async function processUpload(
  ...
  if (uploadError) {
    ...
    if (msg.toLowerCase().includes("bucket") && msg.toLowerCase().includes("not found")) {
      return { error: `Bucket "${bucket}" not found. Create it in Supabase Storage and set SUPABASE_STORAGE_BUCKET.` };
    }
    return { error: msg };
  }
