...
export async function listPartnersRepo(filters: { organizationId?: string } = {}) {
  const where = {
    role: "partner" as const,
    ...(filters.organizationId ? { members: { some: { organizationId: filters.organizationId } } } : {}),
  };
  return prisma.user.findMany({
    where,
...
export async function listPartnersPaginatedRepo(params: { page: number; pageSize: number; organizationId?: string }) {
  const { page, pageSize, organizationId } = params;
  const skip = (page - 1) * pageSize;

  const where: { role: "partner"; members?: { some: { organizationId: string } } } = { role: "partner" };
...
export async function ensureOrganizationAndMemberForPartner(userId: string) {
  ...
  const orgName = user.profile?.entityName?.trim() || user.name?.trim() || "Partner";
