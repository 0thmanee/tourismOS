...
export async function listMyCustomers(): Promise<CustomerListRow[]> {
  const organizationId = await requireProducerOrganizationId("/producer/customers");
  return listCustomersForOrgRepo(organizationId);
}

export async function getMyCustomerDetail(customerId: string): Promise<CustomerDetailRow | null> {
  const organizationId = await requireProducerOrganizationId("/producer/customers");
