...
async function formFileToPayload(formFile: unknown): Promise<UploadPayload | null> {
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to parse request body.";
    ...
    if ("error" in result) {
      const status =
        ...
        : result.error.includes("not found") ? 502
        : result.error.includes("Invalid") || result.error.includes("empty") || result.error.includes("large") ? 400
        : 500;
